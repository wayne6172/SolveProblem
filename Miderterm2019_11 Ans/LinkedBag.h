...
void LinkedBag<ItemType>::copy(const LinkedBag<ItemType>* anotherBag) {
	...
	//這裡不能用add偷吃步，順序應該會有問題
	Node<ItemType>* copyHead = anotherBag->headPtr;
	Node<ItemType>* currPtr = nullptr;
	Node<ItemType>* newNode;

	while (copyHead != nullptr) {
		newNode = new Node<ItemType>(copyHead->getItem());
...
void LinkedBag<ItemType>::compact() {
	...
	Node<ItemType> *prePtr, *currPtr;

	while (checkNode != nullptr) {
		prePtr = checkNode;
		currPtr = checkNode->getNext();

		while (currPtr != nullptr) {
			if (currPtr->getItem() == checkNode->getItem()) {
				prePtr->setNext(currPtr->getNext());
				delete currPtr;

				currPtr = prePtr->getNext();
				itemCount--;
			}
			else {
				prePtr = currPtr;
